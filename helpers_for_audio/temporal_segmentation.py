...
def temporal_segmentation_single_file(file:Path,output_dir:Path, segment_duration:int=5)->None:
    ...
    if(duration>segment_duration):
        ...
        for i in range(no_of_segments):
            ...
            segment_file=output_dir / Path(f"{file.name.removesuffix('.wav')}_{i}.wav")
            try:
...
def temporal_segmentation_single_species(files:list[Path], output_dir:Path, segment_duration:int=5)->None:
    ...
    for i in tqdm(range(files_len)):
        file_count+=temporal_segmentation_single_file(files[i],output_dir=output_dir,segment_duration=5)
        if(file_count>=300):
            break
    print("Segmentation for: {output_dir.name} complete")

def temporal_segmentation_all_data(parent_dir:Path, output_dir:Path, segment_duration:int=5):
    birds_dir_path=[Path(parent_dir) / item for item in os.listdir(parent_dir)  ]
    wav_files_path=[]

    for i in range(len(birds_dir_path)):
        wav_files_path.append([birds_dir_path[i] / item for item in os.listdir(birds_dir_path[i])])

    len_birds=len(wav_files_path)
    for i in tqdm(range(len_birds)):
        temporal_segmentation_single_species(files=wav_files_path[i],output_dir=output_dir,segment_duration=5)
